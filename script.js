...
const Editor = {
  ...
  getElements: function () {
    ...
    this.elements.customCssContainer = document.getElementById(
    ...
    this.elements.customStyleTag = document.getElementById(
    ...
    this.elements.autosaveIndicator =
      document.getElementById("autosave-indicator");

    if (
      !this.elements.textarea ||
      !this.elements.previewContent ||
      !this.elements.previewPane
    ) {
    ...
    if (typeof marked !== "undefined") {
      ...
      markedRenderer.code = (token) => {
        ...
        if (token && typeof token === "object") {
          lang = token.lang || "";
          codeText = token.text || token.raw || "";
        } else {
          codeText = String(token || "");
        }

        return this.handleCodeHighlighting(codeText, lang);
      };
  ...
  setupEventListeners: function () {
    ...
    this.elements.markdownItBtn.addEventListener("click", () =>
      this.SetMarkdownEngine("markdown-it"),
    );
    this.elements.markedBtn.addEventListener("click", () =>
      this.SetMarkdownEngine("marked"),
    );
    this.elements.mathJaxBtn.addEventListener("click", () =>
      this.SetMathEngine("mathjax"),
    );
    this.elements.kaTeXBtn.addEventListener("click", () =>
      this.SetMathEngine("katex"),
    );
    this.elements.downloadPdfBtn.addEventListener("click", () =>
      this.DownloadAs("pdf"),
    );
    this.elements.downloadMdBtn.addEventListener("click", () =>
      this.DownloadAs("md"),
    );
    this.elements.downloadTxtBtn.addEventListener("click", () =>
      this.DownloadAs("txt"),
    );
    this.elements.toggleCssBtn.addEventListener(
      "click",
      this.ToggleCustomCSS.bind(this),
    );
    this.elements.applyCssBtn.addEventListener(
      "click",
      this.ApplyCustomCSS.bind(this),
    );
    this.elements.closeCssBtn.addEventListener(
      "click",
      this.ToggleCustomCSS.bind(this),
    );
  },
  ...
  initializeResponsiveUI: function () {
    ...
    if (this.elements.showEditorBtn && this.elements.showPreviewBtn) {
      this.elements.showEditorBtn.addEventListener("click", () =>
        this.SetMobilePane("editor"),
      );
      this.elements.showPreviewBtn.addEventListener("click", () =>
  ...
  CheckLibraries: function () {
    ...
    if (
      ...
      typeof mermaid.mermaidAPI !== "undefined"
    ) {
  ...
  AllLibrariesReady: function () {
    return (
      (this.state.libsReady.markdownIt || this.state.libsReady.marked) &&
  ...
  UpdatePreview: function (force = false) {
    ...
    if (!force && text === this.state.lastText && this.state.lastText !== "")
      return;

    try {
      const scrollPercent =
        this.elements.previewPane.scrollTop /
        (this.elements.previewPane.scrollHeight -
          this.elements.previewPane.clientHeight);

      if (
        this.state.currentMarkdownEngine === "markdown-it" &&
        this.state.libsReady.markdownIt
      ) {
        ...
        this.state.currentMarkdownEngine === "marked" &&
        this.state.libsReady.marked
      ) {
        ...
        if (this.state.libsReady.markdownIt) {
          ...
          this.elements.previewContent.innerHTML =
            "<p>Error: No valid markdown renderer available</p>";
          return;
        }
      }
  ...
  ConvertMarkedMathDelimiters: function (text) {
    if (typeof text !== "string") return text;
    console.log("Converting marked math delimiters");
    return text
      .replace(/\\\[(.*?)\\\]/gs, "$$ $1 $$")
      .replace(/\\\((.*?)\\\)/gs, "$ $1 $");
  },
  ...
  RenderWithMarked: function (text, scrollPercent) {
    ...
    try {
      const normalizedText = this.ConvertMarkedMathDelimiters(text);
      const html = marked.parse(normalizedText);
      this.elements.previewContent.innerHTML = html;
      this.state.lastRenderedHTML = html;

      if (
        this.state.currentMathEngine === "mathjax" &&
        this.state.libsReady.mathJax &&
      ...
      console.error("Error during marked rendering:", err);
  ...
  _restoreScrollPosition: function (scrollPercent) {
    requestAnimationFrame(() => {
      ...
      const newScrollTop =
        scrollPercent *
        (newScrollHeight - this.elements.previewPane.clientHeight);
      if (
        isFinite(scrollPercent) &&
        newScrollHeight > this.elements.previewPane.clientHeight
      ) {
  ...
  ProcessMath: function () {
    ...
    try {
      if (
        this.state.currentMathEngine === "katex" &&
        this.state.libsReady.katex
      ) {
  ...
  ProcessMermaid: function () {
    ...
    const mermaidBlocks =
      this.elements.previewContent.querySelectorAll("pre.mermaid");
  ...
  SetMobilePane: function (pane) {
    ...
    if (this.elements.showEditorBtn && this.elements.showPreviewBtn) {
      ...
      this.elements.showPreviewBtn.classList.toggle(
        "active",
        pane === "preview",
      );
    }
    ...
    if (this.elements.textarea && this.elements.textarea.parentElement) {
      this.elements.textarea.parentElement.style.display =
        pane === "editor" ? "flex" : "none";
    }

    if (this.elements.previewPane) {
      this.elements.previewPane.style.display =
        pane === "preview" ? "flex" : "none";
    }
  ...
  SetMarkdownEngine: function (engine) {
    if (engine !== this.state.currentMarkdownEngine) {
      ...
      this.elements.markdownItBtn.classList.toggle(
        "active",
        engine === "markdown-it",
      );
      ...
      const mobileMarkdownItBtn = document.getElementById(
        "btn-markdown-it-mobile",
      );
      const mobileMarkedBtn = document.getElementById("btn-marked-mobile");
      if (mobileMarkdownItBtn && mobileMarkedBtn) {
        mobileMarkdownItBtn.classList.toggle(
          "active",
          engine === "markdown-it",
        );
  ...
  ToggleCustomCSS: function () {
    ...
    this.elements.customCssContainer.style.display = this.state.customCssVisible
      ? "flex"
      : "none";
    this.elements.toggleCssBtn.textContent = this.state.customCssVisible
      ? "Hide CSS"
      : "Custom CSS";
  ...
  _generatePdf: async function (filename) {
    if (typeof html2canvas === "undefined" || typeof jspdf === "undefined") {
      alert(
        "PDF generation libraries not loaded yet. Please try again in a moment.",
      );
      return;
    }
    ...
    const downloadBtn =
      ...
      document.getElementById("btn-download-pdf-mobile");
    if (downloadBtn) {
